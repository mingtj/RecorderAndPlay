package com.bajintech.karaokrecorder.record;
...
import com.bajintech.karaokrecorder.callback.RecordStreamListener;
...
public class AudioRecorder {
    ...
    private final static int AUDIO_SAMPLE_RATE = 44100;//16000
    //声道 单声道 CHANNEL_IN_STEREO为双声道，CHANNEL_CONFIGURATION_MONO为单声道
    private final static int AUDIO_CHANNEL = AudioFormat.CHANNEL_IN_STEREO;//CHANNEL_IN_MONO
    //编码 设置音频数据格式:PCM 16位每个样本，保证设备支持。PCM 8位每个样本，不一定能得到设备的支持。
    ...
    public void createAudio(String fileName, int audioSource, int sampleRateInHz, int channelConfig, int audioFormat) {
        // 根据AudioRecord的音频采样率，音频录制声道，音频数据格式获得缓冲区字节大小
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (PcmToWav.mergePCMFilesToWAVFile(filePaths, FileUtils.getWavFileAbsolutePath(fileName))) {
                    //操作成功
                	System.out.println("mergePCMFilesToWAVFile success");
                } else {
                    //操作失败
